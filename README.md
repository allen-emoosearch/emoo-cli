...
> 不加 `--max-results` 时超过 500 条会在 stderr 输出截断警告。需要拉取超过 500 条数据，请用 `emoo data get`（游标翻页，无此限制）。

**自动翻页行为**：加 `--max-results` 后 CLI 自动逐页请求直到达到目标或数据耗尽。
翻页过程中 stderr 实时输出进度：`已获取 N 条 (第 X 页)`。
翻页结束后响应中自动附加标志字段（见 [响应标志](#响应标志)）。
...
| `--text-format` | 否 | plain | `plain` 或 `markdown`（语雀 HTML） |
| `-f, --filter` | 否 | — | 过滤条件，格式同 search |
| `--max-results` | 否 | — | 最多返回条数，游标自动翻页直到拿满或数据源耗尽 |
| `--stream` | 否 | — | 流式输出 JSON Lines (NDJSON)，适合大数据量管道处理 |

> **提示**: 需要拉取超过 500 条数据时，用 `data get` + 日期过滤分段拉取，
> 比 `data search` 更可靠（search 端点有 500 硬上限）。

**自动翻页行为**：加 `--max-results` 后 CLI 用游标逐页请求直到达到目标或 `has_more=false`。
stderr 实时输出进度：`已获取 N 条 (第 X 页)`。翻页结束后响应中附加标志字段。
...
# 带过滤条件
emoo --user-id <id> data get --page-size 10 \
  -f '[[{"field":"app_updated_at","operator":"gte","value":"2025-01-01T00:00:00+08:00"}]]'

# 自动翻页拉取大量数据 (无 500 硬上限)
...
# JSON 输出 + jq 管道处理
emoo --json data get --max-results 10000 | jq '.data.results[] | {title, app_created_at}'
```

**`--stream` 流式模式**：输出 JSON Lines 格式（每行一条 JSON 记录），适合大数据量管道处理。
stdout 逐行输出记录，stderr 输出进度，互不干扰。

```bash
# 流式输出全部数据 (直到数据源耗尽)
emoo --user-id <id> data get --stream
# stderr: 流式模式: 将拉取全部数据直到数据源耗尽...
# stderr:   已输出 200 条 (第 1 页)
# stderr:   已输出 400 条 (第 2 页)
# stdout: {"id":1,"title":"...","app_created_at":"..."}
# stdout: {"id":2,"title":"...","app_created_at":"..."}

# 流式输出 + 限制条数
emoo --user-id <id> data get --stream --max-results 5000

# 流式管道处理：逐行过滤和计算，内存友好
emoo --user-id <id> data get --stream | while read line; do
  echo "$line" | jq -r '.title'
done

# 流式 + 聚合：统计各 app 的文档数
emoo --user-id <id> data get --stream | \
  jq -r '.ws_app.title' | sort | uniq -c | sort -rn

# 流式 + 过滤：只保留特定 app 的记录
emoo --user-id <id> data get --stream | \
  jq -c 'select(.ws_app.title | test("POS"))' > pos_records.jsonl

# 流式 + 日期过滤：按时间段分段拉取
emoo --user-id <id> data get --stream \
  -f '[[{"field":"app_created_at","operator":"gte","value":"2026-04-01T00:00:00+08:00"},{"field":"app_created_at","operator":"lte","value":"2026-04-30T23:59:59+08:00"}]]'
```

> **注意**: `--stream` 不加 `--max-results` 时会一直拉取直到数据源返回 `has_more=false`，
> 可能耗时较长。建议先用 `--max-results` 试探数据量，或配合日期过滤缩小范围。
...
执行流程：模板替换 → app/doc_group 名称解析（知识图谱） → 时间过滤生成 → POST /search（自动翻页） → 可选 CSV 导出。

`skill run` 会自动翻页直到达到设定上限或 API 返回空，stderr 实时输出进度。若命中 search 500 硬上限，结果中会包含 `_truncated` 标志和错误提示。
...
三个数据获取路径都支持自动翻页，行为各有不同：

| 命令 | 翻页方式 | 硬上限 | 何时停止 |
|------|----------|:------:|------|
| `data search --max-results N` | `current_page` 递增 | **500 条** | 达到 N 或 API 返回空或 `total` 耗尽 |
| `data get --max-results N` | `cursor` 游标 | **无** | 达到 N 或 `has_more=false` |
| `skill run` | `current_page` 递增 | **500 条** | 达到 skill 设定上限或 API 返回空 |

所有自动翻页路径都会在 **stderr** 实时输出进度：
```
已获取 200 条 (第 1 页)
已获取 400 条 (第 2 页)
...
...
JSON 输出（`--json`）中，`data` 对象会附加以下标志字段（不影响 API 原始字段）：

| 标志 | 类型 | 含义 |
|------|------|------|
| `_api_total` | int | API 返回的原始 `total` 值（search 端点最大 500） |
| `_paginated` | bool | 是否执行了多页翻页（仅 `page > 1` 时出现） |
| `_page_count` | int | 实际翻页次数（仅多页时出现） |
| `_truncated` | bool | 达到 search 端点 500 硬上限，结果可能不完整（仅 search） |
| `_incomplete` | bool | `max_results` 提前截断，数据源还有更多数据（仅 data get） |

**示例：search 命中 500 硬上限**
...
- **stdout**：数据输出（JSON 或 Rich 表格），`--json` 模式下为纯 JSON
- **stderr**：进度信息、警告、错误提示

```bash
# 管道处理：stdout 走管道，stderr 显示在终端
emoo --json data get --max-results 5000 2>progress.log | jq '.data.results' > output.json

# 流式模式：stdout 逐行 JSON，可直接管道
emoo data get --stream 2>progress.log | jq -c 'select(.ws_app.title | test("POS"))'
```
...
| `--stream` | JSON Lines (NDJSON)，每行一条记录，适合大数据量流式管道处理 |
...
EMOO CLI 的 stdout/stderr 分离设计使其非常适合管道式数据分析：

```bash
# 模式 1: 拉取全部数据后用 jq 聚合
emoo --json data get --max-results 10000 > /tmp/data.json
jq '.data.results' /tmp/data.json | jq -s '
  group_by(.ws_app.title) |
  map({app: .[0].ws_app.title, count: length}) |
  sort_by(-.count)
'

# 模式 2: 流式拉取 + 实时聚合 (内存友好，适合超大数据集)
emoo data get --stream 2>/dev/null | \
  jq -r '"\(.ws_app.title)\t\(.title)\t\(.app_created_at)"' | \
  awk -F'\t' '{app[$1]++} END {for (a in app) print app[a], a}' | \
  sort -rn

# 模式 3: 按月份分段拉取 (突破 search 500 上限)
for month in 01 02 03 04 05 06; do
  emoo --json data get --max-results 10000 \
    -f "[[{\"field\":\"app_created_at\",\"operator\":\"gte\",\"value\":\"2026-${month}-01T00:00:00+08:00\"},{\"field\":\"app_created_at\",\"operator\":\"lte\",\"value\":\"2026-${month}-31T23:59:59+08:00\"}]]"
done | jq -s 'add'
```
...
jq '.data.results' /tmp/q2_revenue.json | jq -s '
  group_by(.title | split("_")[0]) |
  map({
    store: .[0].title | split("_")[0],
    revenue: (map(.content | fromjson.实收合计 // 0 | tonumber) | add)
  }) |
  sort_by(-.revenue) | .[0:10] |
  to_entries | map({rank: (.key+1), store: .value.store, revenue: .value.revenue})
'

# Step 3: 用 skill 一键完成 (替代手写 Step 1-2)
emoo skill run store-revenue --store "所有" --month "2026-Q2"
```
...
# Step 1: 拉取品项销售数据
emoo --json data get --max-results 10000 \
  -f '[[{"field":"app_created_at","operator":"gte","value":"2026-04-01T00:00:00+08:00"},{"field":"app_created_at","operator":"lte","value":"2026-06-30T23:59:59+08:00"}]]' \
  > /tmp/q2_items.json

# Step 2: jq 解析 content JSON，按销售金额排名 Top 30 (排除外卖)
jq '.data.results' /tmp/q2_items.json | jq -s '
  map(.content | fromjson? | select(.菜品名称 != null)) |
  flatten |
  group_by(.菜品名称) |
  map({
    name: .[0].菜品名称,
    amount: (map(.销售金额 // 0 | tonumber?) | add)
  }) |
  map(select(.name | test("外卖") | not)) |
  sort_by(-.amount) | .[0:30] |
  to_entries | map({rank: (.key+1), name: .value.name, amount: .value.amount})
'

# Step 3: 流式处理版本 (内存更低)
emoo data get --stream 2>/dev/null \
  -f '[[{"field":"app_created_at","operator":"gte","value":"2026-04-01T00:00:00+08:00"}]]' | \
  jq -c 'select(.content | fromjson? | .菜品名称 != null)' | \
  jq -s 'group_by(.content | fromjson.菜品名称) | ...'
```

### jq 注意事项

- **中文字段名**：jq 不支持中文裸键名，必须用字符串形式：`."菜品名称"` 而非 `{菜品名称: ...}`
- **类型守卫**：对可能非字符串的字段加类型检查：`select(.菜品名称 | type == "string")`
- **tonumber 容错**：数值字段可能为空或非数字，用 `tonumber?` 静默忽略
- **fromjson 容错**：`content` 字段可能不是合法 JSON，用 `fromjson?` 避免中断
...
emoo data search -k <keyword>              搜索数据 (分页+过滤+markdown+自动翻页)
emoo data get [--max-results N] [--stream] 游标分页获取数据 (无 500 上限 + 流式输出)
...
emoo base table-create -n <name>           创建数据表
emoo base table-list                       获取表列表
emoo base table-update                     更新数据表
emoo base table-delete                     删除数据表
emoo base table-get                        获取表详情
